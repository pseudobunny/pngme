    ...
    fn try_from(data: &[u8]) -> Result<Self> {
        ...
        if header != Png::STANDARD_HEADER {
            return Err(Error::from(
                "The header for the png does not match the standard header.",
            ));
        }
        ...
        let mut i = 8;
        while i < data.len() {
            let chunk = Chunk::try_from(&data[i..])?;
...
impl fmt::Display for Png {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let output: String = self
            .chunks
            .iter()
            .flat_map(|c| c.type_and_data_as_string())
            .map(|c_string| c_string + "\n")
            .collect();
...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        ...
        let chunk_position = self
            .chunks
            .iter()
            .position(|c| *c.chunk_type() == test_type)
            .ok_or(Error::from(
                "Could not find a chunk of this type in the Png",
            ))?;

        Ok(self.chunks.remove(chunk_position))
    }
